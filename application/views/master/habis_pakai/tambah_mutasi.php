...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Tambah Mutasi Bahan Habis Pakai
            <small>Preview</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo base_url() ?>Dashbord"><i class="fa fa-dashboard"></i> Dashbord</a></li>
            <li class="active">Tambah Mutasi Bahan Habis Pakai</li>
        </ol>
    </section>

    <!-- Main content -->
                  <form class="form-horizontal" method="post" action="<?php echo base_url()?>BahanHabisPakai/tambahMutasi">
...
<script type="text/javascript">
    ...
    function getObatData(){
      ...
      $.ajax({
        url: '<?php echo base_url()?>BahanHabisPakai/getDataBahanDiGudang',
        method: 'post',
        dataType: 'json',
        data: {
          id: id_obat
        ...
        success: function(data){
          if (data.length > 0) {
              ...
              var button = ( avaliable.length > 0 ? '<i class="text-warning">Sudah ditambahkan</i>' : `<button type="button" class="btn btn-sm btn-primary btn-select" data-rowid="` + key + `"><i class="fa fa-plus"></i> Tambahkan</button>` );
                ...
                rows = rows + `
                  <tr data-rowid="` + key + `">
                    ...
                    <td class="text-right">`+ value.jumlah +` `+ value.satuan +`</td>
            ...
            $('#form-obat .btn-select').on('click', function(){
              ...
              var row = `
                <tr class="row-obat" data-rowid="` + value.id + `">
                  ...
                  <td>
                    <span class="name">`+ value.nama +`</span>
                  </td>
                  <td class="text-right">`+ value.jumlah +` `+ value.satuan +`</td>
                  <td>
            ...
            selector.parent('div').append('<span class="small-error text-danger"><i class="fa fa-exclamation-circle"></i> Bahan habis pakai tersebut belum ada digudang.</span>') ;
            initModalTable();
          }
        },
        error: function(data){
